...
def solve_quadprog(hessian, weights, eq_coeffs, eq_constants, ineq_coeffs, ineq_constants, lower_bounds, upper_bounds):
    ...
    if ineq_coeffs is not None:
        ...
        full_ineq_constants = np.concatenate(
            (-ineq_constants, new_ineq_constants))
    else:
    ...
    # add a small constant to the diagonal of the Hessian to help numerical stability
    small_constant = 1e-8
    print('Started quad prog')
    # solve quadratic program
    try:
        sol = quadprog.solve_qp(
            hessian + small_constant * np.eye(n), weights, coeffs, constants, num_eq)[0]
    except ValueError:
